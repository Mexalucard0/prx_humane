...
RegisterServerEvent('prx_humane:toofar')
AddEventHandler('prx_humane:toofar', function(robb)
	local source = source
	...
	for i=1, #xPlayers, 1 do
 		...
 		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('prx_humane:killblip', xPlayers[i])
		end
	end
	if(robbers[source])then
		TriggerClientEvent('prx_humane:toofarlocal', source)
		robbers[source] = nil
		TriggerClientEvent('esx:showNotification', source, _U('robbery_has_cancelled') .. Banks[robb].nameofbank)
	end
end)

RegisterServerEvent('prx_humane:rob')
AddEventHandler('prx_humane:rob', function(robb)
			...
			if(cops >= Config.NumberOfCopsRequired)then
				...
				for i=1, #xPlayers, 1 do
					...
					if xPlayer.job.name == 'police' then
							...
							TriggerClientEvent('prx_humane:setblip', xPlayers[i], Banks[robb].position)
					end
				end
				...
				TriggerClientEvent('prx_humane:currentlyrobbing', source, robb)
				...
				SetTimeout(600000, function()
					...
					if(robbers[savedSource])then
						...
						rob = false
						TriggerClientEvent('prx_humane:robberycomplete', savedSource, job)
						if(xPlayer)then
